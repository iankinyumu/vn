...
Reviewed against the local source on 15 September 2026; item statuses were revised on 20 September 2026 after the order-flow hardening pass. This is a code review, not confirmation of the deployed database configuration.
...
- The quote refresh function also triggers demo matching, and the scheduled worker derives its watchlist from the market registry (`list_executable_symbols`) rather than a fixed BTCUSDT/ETHUSDT list, so it covers every pair the interface can offer. Matching on the quote path is best effort: a failure there is logged and swallowed so the customer still receives a quote.
...
4. **Independent market data.** Quote fetching is now one shared module (`supabase/functions/_shared/market-data.mjs`) with per-host failover and book validation, and the demo worker covers every registry-tradable pair, so extraction and symbol coverage are done. What remains is running ingestion as a consumer independent of the request path, instead of on demand from a user quote.
...
6. **Stable retries.** Use account-scoped idempotency keys, retain the same key across retries, and reject changed payloads under the same key. The database already has account-scoped uniqueness, and the browser now keeps one idempotency key per order intent — reusing it for an unchanged payload and minting a new one as soon as any order field changes. The demo RPC still returns an existing order without checking payload equality, so a changed payload under a reused key would be accepted silently rather than rejected.
