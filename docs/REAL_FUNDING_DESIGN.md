...
- **Sandbox numbers only.** In sandbox, `funding_svc_begin_payment` pushes only to MSISDNs in `funding.sandbox_msisdns` (seeded with the Daraja sandbox test number), or to a number the paying tester registered for themselves in `funding.sandbox_tester_msisdns` (migration `20260926120000`). A real customer's phone therefore never receives a sandbox prompt: only an owner-enabled tester can register a number, and only for their own deposits.
